// POS System - Professional Data Layer with Firestore persistence
// Inspired by SambaPOS architecture
...
const DEFAULT_STAFF = [
  { id: 'staff_admin', name: 'Admin', pin: '1234', role: 'admin', active: true },
  { id: 'staff_kassier', name: 'Kassier', pin: '0000', role: 'kassier', active: true },
];
...
export async function startWorkPeriod(startCash, staffId) {
  const period = {
    id: `WP-${Date.now()}`,
    startTime: Date.now(),
...
export async function endWorkPeriod(endCash, staffId) {
...
export async function getWorkPeriodHistory() {
  try {
    const snap = await getDocs(
      query(collection(db, 'workPeriods'), orderBy('startTime', 'desc'))
    );
    return snap.docs.map(d => ({ ...d.data(), _docId: d.id }));
  } catch (err) {
...
// ===== PARKED ORDERS (Geparkeerde bestellingen) =====
...
// ===== ENHANCED ORDER with discount & notes =====
