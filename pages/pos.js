...
import { getTodayOrders, formatPrice, getStaff, verifyPin, getActiveWorkPeriod, getParkedOrders, getBusinessSettings, hasPermission, getPendingIncomingOrders } from '../lib/pos-data';
import { auth } from '../lib/firebase';
import { signOut } from 'firebase/auth';

export default function PosHome() {
  const router = useRouter();
  const [stats, setStats] = useState({ todayOrders: 0, todayRevenue: 0, activeOrders: 0, parkedOrders: 0, avgOrder: 0, itemsSold: 0, pendingIncoming: 0 });
  ...
  useEffect(() => {
    const updateStats = async () => {
      const [todayOrders, period, parked, bs, pendingInc] = await Promise.all([
        getTodayOrders(),
        getActiveWorkPeriod(),
        getParkedOrders(),
        getBusinessSettings(),
      ...
      setStats({
        ...
        pendingIncoming: pendingInc.length,
      });
  ...
  const tiles = [
    { title: 'Kassa', icon: '🧾', desc: 'Nieuwe bestelling opnemen', color: '#e74c3c', href: '/kassa', badge: stats.parkedOrders > 0 ? `${stats.parkedOrders} geparkeerd` : stats.pendingIncoming > 0 ? `${stats.pendingIncoming} inkomend` : null },
    ...
    { title: 'Bezorging', icon: '🚚', desc: 'Koeriers, routes & tracking', color: '#FF8000', href: '/bezorging' },
    ...
    { title: 'Integraties', icon: '🔗', desc: 'Takeaway.com & Odoo', color: '#e91e63', href: '/integraties' },
  ];
